...
def save_stats(env, t, learning_starts, exploration):
...
def dqn_learing(
    ...
    # input_arg = env.observation_space.shape
    input_arg = 3 #number of channels in input space
    ...
    ###############
    num_param_updates = 0
    last_obs = env.reset(num_solid_objects=np.random.choice([1,2]))
    ...
    for t in count():
        ...
        obs, reward, done, _ = env.act(action)
        ...
        save_stats(env, t, learning_starts, exploration)

        # Resets the environment when reaching an episode boundary.
        if done:
            obs = env.reset(num_solid_objects=np.random.choice([1,2]))
            game_count+=1

        if game_count % DRAW_EVERY_X_GAMES == 0:
            outI2 = cv2.resize(obs, draw_shape)
            cv2.imshow('Maze State', outI2)
            cv2.waitKey(1)
